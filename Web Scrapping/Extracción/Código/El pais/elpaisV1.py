...
class program:
    ...
    def inArticle(soup):
        try:
            ...
            aux = ""
            divBody = soup.find('div', class_='a_c clearfix')
            paragraphs = divBody.find_all('p')
            for paragraph in paragraphs:
                if paragraph.get_text(strip=True) != 'O suscríbete para leer sin límites':
                    aux+= paragraph.text
            return f'{subtitle}. {aux}'
